...
function EventScreen() {
  ...
  return (
    <div className={`bg-mountain-800/80 backdrop-blur rounded-lg border h-full flex flex-col card-atmosphere ${
      ...
      <div className={`p-4 sm:p-5 border-b ${isMemorial ? 'border-purple-500/20' : config.border}`}>
        <div className="flex items-center gap-2">
          <Icon size={18} className={config.color} />
          <h2 className={`text-lg font-bold ${config.color}`}>{event.title}</h2>
          <span className={`text-xs px-2 py-0.5 rounded-full border ${config.border} ${config.color} bg-current/5`}>{config.label}</span>
          {isMemorial && <span className="text-xs bg-purple-500/15 text-purple-300 px-2 py-0.5 rounded">真实事故纪念</span>}
        </div>
        <p className="mt-3 text-sm text-mountain-200 leading-relaxed story-text">{event.description}</p>
      </div>
      ...
      <div className="flex-1 p-4 sm:p-5 overflow-y-auto">
        ...
        <div className="space-y-2.5">
          {event.choices.map((choice, idx) => (
            <button key={choice.id} onClick={() => handleEventChoice(choice.id)}
              className={`choice-btn w-full text-left p-3.5 rounded-lg border transition-all duration-250 cursor-pointer ${config.bg} ${config.border} hover:brightness-150 hover:scale-[1.01]`}>
        ...
        {isMemorial && (
          <button onClick={skipEvent} className="mt-3 flex items-center gap-1.5 px-3 py-1.5 bg-mountain-700/60 hover:bg-mountain-600/80 border border-mountain-600/50 rounded-md text-xs text-mountain-400 transition-all cursor-pointer">
